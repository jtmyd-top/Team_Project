...
export const useUserStore = defineStore('user', {
  state: () => {
    ...
    return {
      ...
      sidebar: {
        ...
        activeTab: localStorage.getItem('sidebarActiveTab') || 'all', // 'all', 'spaces', 'favorites', 'private', 'trash', 'settings'
        expandedFolderIds: JSON.parse(localStorage.getItem('expandedFolderIds') || '[]')
      }
    };
  },
  ...
  actions: {
    ...
    toggleSidebar() {
      ...
      localStorage.setItem('isSidebarCollapsed', this.sidebar.isCollapsed);
    },
    ...
    toggleFolderExpanded(folderId) {
      const index = this.sidebar.expandedFolderIds.indexOf(folderId);
      if (index === -1) {
        this.sidebar.expandedFolderIds.push(folderId);
      } else {
        this.sidebar.expandedFolderIds.splice(index, 1);
      }
      localStorage.setItem('expandedFolderIds', JSON.stringify(this.sidebar.expandedFolderIds));
    },
  ...
  getters: {
    ...
    activeTab: (state) => state.sidebar.activeTab,
    expandedFolderIds: (state) => state.sidebar.expandedFolderIds
  }
})
