...
const y="modulepreload",k=function(u){return"/static/dist/"+u},w={},v=function(g,c,b){let p=Promise.resolve();if(c&&c.length>0){document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),e=(o==null?void 0:o.nonce)||(o==null?void 0:o.getAttribute("nonce"));p=Promise.allSettled(c.map(t=>{if(t=k(t),t in w)return;w[t]=!0;const s=t.endsWith(".css"),n=s?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${t}"]${n}`))return;const a=document.createElement("link");if(a.rel=s?"stylesheet":y,s||(a.as="script"),a.crossOrigin="",a.href=t,e&&a.setAttribute("nonce",e),document.head.appendChild(a),s)return new Promise((l,h)=>{a.addEventListener("load",l),a.addEventListener("error",()=>h(new Error(`Unable to preload CSS for ${t}`)))})}))}function r(o){const e=new Event("vite:preloadError",{cancelable:!0});if(e.payload=o,window.dispatchEvent(e),!e.defaultPrevented)throw o}return p.then(o=>{for(const e of o||[])e.status==="rejected"&&r(e.reason);return g().catch(r)})};document.addEventListener("DOMContentLoaded",async function(){const{convertUbbMarkupInHtml:u,renderCommentUbb:g,hydrateUbbDom:c}=await v(async()=>{const{convertUbbMarkupInHtml:e,renderCommentUbb:t,hydrateUbbDom:s}=await import("./chunks/ubb-DLUgd71s.js");return{convertUbbMarkupInHtml:e,renderCommentUbb:t,hydrateUbbDom:s}},__vite__mapDeps([0,1])),{enhanceCodeBlocks:b}=await v(async()=>{const{enhanceCodeBlocks:e}=await import("./chunks/useCodeEnhancer-D0rmz2rR.js");return{enhanceCodeBlocks:e}},[]),{createPublicNoteComments:p}=await v(async()=>{const{createPublicNoteComments:e}=await import("./chunks/usePublicNoteComments-DVB3oicn.js");return{createPublicNoteComments:e}},__vite__mapDeps([2,3])),{getCsrfToken:r}=await v(async()=>{const{getCsrfToken:e}=await import("./chunks/csrf-BCH8yQFZ.js");return{getCsrfToken:e}},[]);if(!document.getElementById("pn-author-card-inline-styles")){const e=document.createElement("style");e.id="pn-author-card-inline-styles",e.textContent=`
    ...
    `,document.head.appendChild(e)}if(window.GLOBAL_DATA={noteData:null,navigationData:null,isAuthenticated:!1},function(){const e=document.getElementById("navigation-data"),t=document.getElementById("note-data");if(e)try{const s=JSON.parse(e.textContent);window.GLOBAL_DATA.navigationData=s,window.GLOBAL_DATA.isAuthenticated=s.is_authenticated||!1}catch(s){console.error("解析导航数据失败:",s)}if(t)try{window.GLOBAL_DATA.noteData=JSON.parse(t.textContent)}catch(s){console.error("解析笔记数据失败:",s)}}(),typeof Vue>"u"){console.error("Vue not loaded");return}const o=Vue.createApp({template:`
      <div v-if="note">
        <!-- 顶部导航栏 -->
        <nav class="pn-navbar">
